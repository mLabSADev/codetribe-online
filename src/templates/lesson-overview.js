...
const CourseOverview = ({data}) => {
    ...
    const [lessonToMoveTo, setLessonToMoveTo] = useState(null)
    ...
    useEffect(() => {
        LessonService.currentLessonPosition(data.markdownRemark.fields.tutorial).then(position => {
            ...
            setLessonToMoveTo(lessonToMoveTo)
...
<div style={{
                        ...
                        }}><h1>{post.frontmatter.title}</h1></div>
                    ...
                    <Button style={{
                        ...
                        borderStyle: 'none'
                    }} onClick={startCourse}>{'View Course'}</Button>
            ...
            <Col  xs={24} sm={24} md={24} lg={24}>
                        <div style={{ width: '100%', maxWidth: '100%', marginBottom: 20, borderRadius: 20}}>
                                    ...
                                    <Collapse style={{ borderStyle: 'none',  borderRadius: 30, background: 'transparent'}} defaultActiveKey={[`${currentChapter}`]} bordered={false}>
                                        {Object.keys(chapters).map(key => {
                                            ...
                                            return (
                                                ...
                                                <Collapse.Panel showArrow={false} expandIconPosition='none' header={<div style={{borderStyle: 'none', background: '#efefef', padding: 20, borderRadius: 20, color: chapter.current ? '#97CA42' : '#606060', fontSize: '1.5em', fontWeight: 'bold'}}>{`${chapter.title} (${chapterTotalDuration})`}</div>} key={key} style={{background: 'transparent', borderColor: '#f0f2f5'}}>
                                                    <div style={{background: '#efefef', marginBottom: 10, padding: 20, borderRadius: 30}}>
